...
| 2026-08-19 | Last verified обновлён для AGENTS.md, CHANGELOG.md [Unreleased], README.md, CONTRIBUTING.md, docs/00, docs/19, .coverage-baseline.txt (кандидат 5 обзора №4 — форматы листа один каталог #113, тикет #114, PR #116: SheetFormatCatalog + record SheetFormat — 11 точек знания стали потребителями; строковая идентичность — ADR-0002; меню генерируется из каталога; мёртвая настройка DefaultSheetFormat оживает fallback'ом; дефолт «A3» ×6 — const DefaultName; диалог — TryGet-no-op: test_count 2630, coverage 90.38%; CODING_STANDARDS без числовых метрик — изменений не потребовал, CONTEXT.md «Стандартный формат»/«Пользовательский формат» добавлены в реализации, исторические спринт-логи не изменялись) | Sprint docs sync (#115) |
| 2026-08-19 | Last verified обновлён для AGENTS.md, CHANGELOG.md [Unreleased], README.md, CONTRIBUTING.md, docs/00, docs/19, CODING_STANDARDS.md, .coverage-baseline.txt (кандидат 6 обзора №4 — TextGeometry из модели #118, тикет #119, PR #121: WPF-знание геометрии повёрнутого текста вынесено в модуль TextGeometry (Helpers: LayoutOffset/Corner/Contains/BoundingBox); модель Text — тонкие делегации (XAML маркеров 0 изменений, INPC-проводка на модели; 393→217 строк); мёртвые Visual*×4 с INPC-обвязкой и RotationAngleValid удалены; 12 геометрических тестов мигрированы в TextGeometryTests; print-дефект обзора проверен и не подтвердился: test_count 2630, coverage 90.34%; CODING_STANDARDS — синхронизирован счётчик тестов (2648→2630, дрейф с эпохи #45–#48), Common Mistakes #82 добавлен, CONTEXT.md/ADR не потребовались (модуль — реализационная структура), исторические спринт-логи не изменялись) | Sprint docs sync (#120) |
| 2026-08-20 | Last verified обновлён для AGENTS.md, CHANGELOG.md [Unreleased], README.md, CONTRIBUTING.md, docs/00, docs/19, .coverage-baseline.txt (кандидат 3 обзора №5 — один шов проверки документа #142, тикет #143, PR #145: обёртка ITemplateService.Validate удалена (интерфейс 5→4 члена), TabOperationsService инжектит ITemplateValidator напрямую (фильтр «только Error» + формат в потребителе побайтно), цепочка цвета сокращена (HexColorValidation напрямую, IValidationService сохранён): test_count 2613 (0 пропусков), coverage 91.13%; CODING_STANDARDS §6.3 синхронизирован фикс-коммитом реализации (находка Standards-ревью), CONTEXT.md «Правила проверки документа» обновлён в реализации, Common Mistakes #84 добавлен, попутно заполнены пропущенные строки спринт-таблицы docs/19 (кандидаты 5/6 обзора №4, кандидат 1 обзора №5), исторические спринт-логи не изменялись) | Sprint docs sync (#144) |
| 2026-08-20 | Last verified обновлён для AGENTS.md, CHANGELOG.md [Unreleased], README.md, CONTRIBUTING.md, docs/00, docs/19, .coverage-baseline.txt (кандидат 4 обзора №5 — добить миграцию тестов библиотек #147, тикет #148, PR #150: тесты библиоточных типов перенесены из app-проекта в Document.Tests/Sheets.Tests — V-правила тремя источниками в один TemplateValidatorTests (75) + HexColorValidationTests (4), новые TemplateTests/MetadataTests/RectMicronsTests/SheetTests, слияния в CoordinateTests/PointMicronsTests/TemplateServiceTests; общая фикстура TestTemplates; app-остатки влиты в родительские, TemplateTests.cs → GridSettingsTests.cs; 5 файлов-источников + 3 реликта удалены; FixedFontMetrics/FontMetricsTestCollection ×2 — осознанные копии: test_count 2613 (0 пропусков; распределение приложение 2085 + Document.Tests 401 + Sheets.Tests 127), coverage 91.13% (без изменений); CONTRIBUTING.md — тестовые команды переведены на весь .slnx (тесты живут в трёх проектах), счётчики проектов синхронизированы, принцип «тесты живут с кодом» добавлен; CODING_STANDARDS/CONTEXT.md не затронуты (чистый перенос тестов, новых терминов нет), исторические спринт-логи не изменялись) | Sprint docs sync (#149) |
| 2026-08-20 | Last verified обновлён для AGENTS.md, CHANGELOG.md [Unreleased], README.md, CONTRIBUTING.md, docs/00, docs/19, .coverage-baseline.txt (кандидат 5 обзора №5 — deletion sweep №2 #152, тикет #153, PR #155: мёртвое знание приложения удалено одним проходом — TextToolSettings (дефолты инлайнены в TextTool), класс Grid (шаг сетки — одна константа, FromDefaultGrid → CreateDefault), настройка-призрак DefaultZoom (settings.json байт-совместим), мёртвые константы DoubleClickThresholdMs/DefaultSheetOffsetMm + NudgeStepMicrons → 100 подключён в NudgeStep (расхождение устранено, поведение сохранено), метки ориентации «кн.»/«алб.» — одна точка OrientationLabels.For (явный throw), Rotate ±90 — приватное ядро со знаком, mojibake восстановлен (86 строк в трёх файлах), SnapHelper.SnapObject удалён с 3 тестами, три теста-реликта перенесены в Document.Tests; поведение байт-в-байт, единственное видимое изменение — строки логов: test_count 2603 (0 пропусков; распределение приложение 2072 + Document.Tests 404 + Sheets.Tests 127), coverage 91.13% (без изменений); CONTRIBUTING.md — счётчики проектов синхронизированы (2072/404/127); CODING_STANDARDS/CONTEXT.md не затронуты (deletion-срез, новых терминов нет), исторические спринт-логи не изменялись) | Sprint docs sync (#154) |
| 2026-08-21 | Last verified обновлён для AGENTS.md, CHANGELOG.md [Unreleased], README.md, CONTRIBUTING.md, docs/00, docs/19, .coverage-baseline.txt (кандидат 2 обзора №5 — формат и проверка знают тип: каталог типов объекта #157, тикет #158, PR #160: ObjectTypeCatalog + ObjectTypeDescriptor в документной библиотеке — единственный владелец строк идентичности «Line»/«Rectangle»/«Text» (ADR-0004), маппинга «запись ↔ модель» и объектных правил проверки (V-003 → V-004 → V-007 → V-005 внутри объекта); TemplateService — ноль switch'ей по типу (дефолты загрузки без изменений); идентификатор объекта переживает «сохранил → загрузил» (отсутствующий/пустой → новый, копия получает новый); объектные правила валидатора — в дескрипторах, ошибки группируются по объекту, один типовой фильтр на уровне шаблона (V-002 ключи); мёртвый ValidateObject удалён — контракт валидации 2 → 1 (6 тестов удалены, сценарии перепокрыты); семантика неизвестного типа побайтно; модель не изменена; поведение байт-в-байт, кроме двух декларированных отклонений (round-trip идентификатора; группировка ошибок по объекту): test_count 2617 (0 пропусков; распределение приложение 2072 + Document.Tests 418 + Sheets.Tests 127), coverage 91.18%; CONTEXT.md — обновлена формулировка термина «Объекты шаблона» (новый тип добавляется одной точкой в каталоге типов документной библиотеки; редакторный чеклист точек сохранён), новый термин не вводился (по grilling Q12); CODING_STANDARDS — синхронизирован счётчик тестов в шапке (2630 → 2617, дрейф с эпохи обзора №4; правил о типе объекта нет — остальное не затронуто); библиотечная часть wayfinder-чеклиста #131 замещена одной точкой в каталоге типов — упомянуто в AGENTS.md/CONTEXT.md/документации среза, комментарий в wayfinder-карте #125; исторические спринт-логи не изменялись) | Sprint docs sync (#159) |
| 2026-08-21 | Last verified обновлён для AGENTS.md, CHANGELOG.md [Unreleased], README.md, CONTRIBUTING.md, docs/00, docs/19, .coverage-baseline.txt (кандидат 7 обзора №5 — предпросмотр печати через шов диалогов и типизированная ориентация меню #167, тикет #168, PR #170: PreviewPrint через IDialogHostService (PrintPreviewViewModel + маппинг, окно с конструктором от модели), владелец диалогов по умолчанию — главное окно (инъектируемый провайдер в WPF-реализации шва, CenterOwner у всех трёх окон), типизированная ориентация меню (NewSheetOrientationEntry), строковый протокол «A4L» и парсер суффиксов удалены целиком, фасад CreateNewTab с типизированной ориентацией (цепочки побайтно), settings.json байт-совместим; байт-в-байт кроме центрирования двух диалогов: test_count 2625 (0 пропусков; распределение приложение 2044 + Document.Tests 454 + Sheets.Tests 127), coverage 92.05%; CONTEXT.md — статья «Предпросмотр печати» (отличается от «Предпросмотра»); CODING_STANDARDS §6.3 — показ окон только через шов IDialogHostService + инъектируемый провайдер владельца (потоко-аффинность WPF-статики), Common Mistakes #85 добавлен; исторические спринт-логи не изменялись) | Sprint docs sync (#169) |
| 2026-08-21 | Last verified обновлён для AGENTS.md, CHANGELOG.md [Unreleased], README.md, CONTRIBUTING.md, docs/00, docs/19, .coverage-baseline.txt (кандидат 8 обзора №5 — темы: одна структура стилей + две палитры вместо двух словарей по 464 строки #172, тикет #173, PR #175: новый ThemeStructure.xaml — общая структура 15 стилей без единого цвета (кисти — DynamicResource на палитру, механизм живой подмены), DarkTheme.xaml/LightTheme.xaml — палитры (по 22 цвета + 22 кисти, файлы различаются только значениями цветов), механика смены темы и её тесты — нулевая правка; мёртвые ключи MaterialFlat* удалены; декларированные отклонения: живая смена темы из Настроек (фикс скрытого дефекта — раньше применялась только при следующем запуске) и кисти в палитрах, а не в структуре (единственный контекст наследования незамороженного Freezable — смоук показал частичное перекрашивание); эффективная ресурсная поверхность запинена ThemeResourceSurfaceTests (STA); байт-в-байт кроме двух декларированных отклонений: test_count 2632 (0 пропусков; распределение приложение 2051 + Document.Tests 454 + Sheets.Tests 127), coverage 91.41%; CONTEXT.md — статья «Палитра» (набор цветов одной темы; структура стилей общая), статья «Тема» проверена — формулировка точна без правок после фикса живой смены из настроек; CODING_STANDARDS — синхронизирован счётчик тестов в шапке (2625 → 2632), правил о темах нет — остальное не затронуто; Common Mistakes #86 добавлен (кисти темы — в палитре рядом со своими цветами: единственный контекст наследования незамороженного Freezable); исторические спринт-логи не изменялись) | Sprint docs sync (#174) |
