...
**Тестов:** 2617, 0 сбоев, 0 пропусков
**Сборка:** 0 errors, 0 warnings
**Покрытие:** 91.18% line-rate (CI gate 80%)
**Последнее обновление:** 21.08.2026 — Кандидат 2 обзора №5 (#157, тикет #158, PR #160): формат и проверка знают тип — каталог типов объекта. Документная библиотека знает свои типы в одной точке: `ObjectTypeCatalog` + `ObjectTypeDescriptor` (идентичность «Line»/«Rectangle»/«Text», маппинг «запись ↔ модель», объектные правила проверки); `TemplateService` — ноль switch'ей по типу; идентификатор объекта переживает «сохранил → загрузил» (отсутствующий/пустой → новый); мёртвый `ValidateObject` удалён (контракт валидации 2 → 1); ADR-0004; модель не изменена, приложение и XAML — ноль изменений; поведение байт-в-байт, кроме двух декларированных отклонений (round-trip идентификатора; группировка ошибок по объекту), тесты 2617 (2603 − 6 тестов удалённого шва + 20 пинов; распределение: приложение 2072 + Document 418 + Sheets 127), coverage 91.18%
...
- ✅ **2617 тестов**, 0 сбоев, 0 пропусков
- ✅ **Покрытие:** 91.18% line-rate (CI gate 80%)
- ✅ **Сборка:** 0 errors, 0 warnings
...
**Последнее обновление:** 21.08.2026 — Кандидат 2 обзора №5 (#157, тикет #158, PR #160): формат и проверка знают тип — каталог типов объекта. Документная библиотека знает свои типы в одной точке: статический `ObjectTypeCatalog` + sealed `ObjectTypeDescriptor` (прецедент `SheetFormatCatalog`, без DI) — единственный владелец строк идентичности «Line»/«Rectangle»/«Text» (ADR-0004), маппинга «запись ↔ модель» и объектных правил проверки документа (V-003 → V-004 → V-007 → V-005 внутри объекта). `TemplateService` делегирует каталогу чтение и запись объектов (ноль switch'ей по типу, дефолты загрузки без изменений); идентификатор объекта переживает «сохранил → загрузил» (отсутствующий или пустой в файле → новый; `Clone()` не меняется — копия получает новый идентификатор). Объектные правила валидатора идут через дескрипторы с группировкой ошибок по объекту; на уровне шаблона один типовой фильтр (V-002 ключи) вместо четырёх каскадов. Мёртвый `ValidateObject` удалён — контракт валидации 2 → 1 член. Семантика неизвестного типа сохранена побайтно (молчаливый пропуск при загрузке; запись без типа при записи неизвестного подтипа). Модель (`TemplateObjectBase`) не изменена вовсе; приложение и XAML — ноль изменений. Поведение байт-в-байт, кроме двух декларированных отклонений: round-trip идентификатора и группировка ошибок по объекту в диалоге сохранения. Библиотечная часть wayfinder-чеклиста #131 замещена одной точкой в каталоге типов; редакторная часть чеклиста остаётся в силе. Тесты 2617 (2603 − 6 тестов удалённого шва + 20 пинов; распределение: приложение 2072 + Document.Tests 418 + Sheets.Tests 127); coverage 91.18%
