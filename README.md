...
**Тестов:** 2655, 0 сбоев, 0 пропусков
**Сборка:** 0 errors, 0 warnings
**Покрытие:** 91.16% line-rate (CI gate 80%)
**Последнее обновление:** 21.08.2026 — Кандидат 6 обзора №5 (#162, тикет #163, PR #165): запасной шрифт — рендер и метрики сходятся в каталоге шрифтов. Неизвестное имя шрифта ведёт себя как шрифт по умолчанию («ГОСТ А») — одинаково в рендере и в геометрии: статический `FontCatalog` + sealed `FontDescriptor` в документной библиотеке — единственный владелец строковой идентичности шрифтов, внутренних имён файлов и запасных коэффициентов; `FallbackFontMetrics` делегирует каталогу целиком, `WpfFontMetrics` и `RenderRules` — потребители (pack-URI по внутреннему имени, кэш `FontFamily`); раунд-трип неизвестного имени байт-совместим; ADR-0003 правлен по месту; «Метрики шрифта» в CONTEXT.md дополнены; известные шрифты байт-в-байт, два декларированных отклонения для неизвестного имени (рендер «ГОСТ А» вместо Segoe UI, ширина 0.5 вместо 0.6), тесты 2655 (2617 + 38 пинов; распределение: приложение 2074 + Document 454 + Sheets 127), coverage 91.16%
...
|------|----------|--------|------|
| **1** | Редактор шаблонов листов | ✅ ЗАВЕРШЁН (2655 тестов, 100%) | Q2 2026 |
...
- ✅ **2655 тестов**, 0 сбоев, 0 пропусков
- ✅ **Покрытие:** 91.16% line-rate (CI gate 80%)
- ✅ **Сборка:** 0 errors, 0 warnings
...
**Последнее обновление:** 21.08.2026 — Кандидат 6 обзора №5 (#162, тикет #163, PR #165): запасной шрифт — рендер и метрики сходятся в каталоге шрифтов. Неизвестное имя шрифта ведёт себя как шрифт по умолчанию («ГОСТ А») — одинаково в рендере и в геометрии: статический `FontCatalog` + sealed `FontDescriptor` в документной библиотеке (прецеденты `SheetFormatCatalog`/`ObjectTypeCatalog`, без DI) — единственный владелец строковой идентичности шрифтов, внутренних имён файлов шрифтов и запасных коэффициентов метрик; `FallbackFontMetrics` делегирует каталогу целиком, `WpfFontMetrics` берёт из каталога внутренние имена и запасные значения (неизвестное имя нормализуется к дефолтному), `RenderRules` строит pack-URI по внутреннему имени и кэширует экземпляры `FontFamily`; неизвестное имя и null в рендере — шрифт по умолчанию. Литерал «ГОСТ А» в конструкторе `Text` заменён единым источником дефолтов. Раунд-трип неизвестного имени байт-совместим (новый пин). ADR-0003 правлен по месту (эмбиентный слот сохраняется, дубль запасных значений заменён одним каталогом); статья «Метрики шрифта» в CONTEXT.md дополнена. Известные шрифты — байт-в-байт; декларированные отклонения только для неизвестного имени: рендер «ГОСТ А» вместо Segoe UI и коэффициент ширины 0.5 вместо 0.6. XAML, конвертеры, модели представления и сервисы приложения — ноль изменений. Тесты 2655 (2617 + 38 пинов; распределение: приложение 2074 + Document.Tests 454 + Sheets.Tests 127); coverage 91.16%
