...
**Тестов:** 2632, 0 сбоев, 0 пропусков
**Сборка:** 0 errors, 0 warnings
**Покрытие:** 91.41% line-rate (CI gate 80%)
**Последнее обновление:** 21.08.2026 — Кандидат 8 обзора №5 (#172, тикет #173, PR #175): темы — одна структура стилей + две палитры вместо двух словарей по 464 строки. Новый `ThemeStructure.xaml` несёт общую структуру всех 15 стилей без единого цвета (кисти — `DynamicResource` на палитру, механизм живой подмены), `DarkTheme.xaml`/`LightTheme.xaml` стали палитрами (по 22 цвета + 22 кисти; файлы различаются только значениями цветов) — механика смены темы и её тесты остались нулевой правкой; мёртвые ключи `MaterialFlat*` удалены; декларированные отклонения: живая смена темы из Настроек (фикс скрытого дефекта — раньше применялась только при следующем запуске) и кисти в палитрах, а не в структуре (единственный контекст наследования незамороженного `Freezable`); эффективная ресурсная поверхность запинена `ThemeResourceSurfaceTests` (STA); CONTEXT.md — статья «Палитра»; байт-в-байт кроме двух декларированных отклонений, тесты 2632 (2625 + 7; распределение: приложение 2051 + Document 454 + Sheets 127), coverage 91.41%
...
|------|----------|--------|------|
| **1** | Редактор шаблонов листов | ✅ ЗАВЕРШЁН (2632 теста, 100%) | Q2 2026 |
...
- ✅ **2632 теста**, 0 сбоев, 0 пропусков
- ✅ **Покрытие:** 91.41% line-rate (CI gate 80%)
- ✅ **Сборка:** 0 errors, 0 warnings
- ✅ **#172–#173:** Кандидат 8 обзора №5 — темы: одна структура стилей + две палитры вместо двух словарей по 464 строки: новый `ThemeStructure.xaml` — общая структура 15 стилей без единого цвета (кисти — `DynamicResource` на палитру), `DarkTheme.xaml`/`LightTheme.xaml` — палитры (по 22 цвета + 22 кисти, файлы различаются только значениями цветов), механика смены темы и её тесты — нулевая правка, мёртвые ключи `MaterialFlat*` удалены; декларированные отклонения: живая смена темы из Настроек (фикс скрытого дефекта) и кисти в палитрах, а не в структуре (единственный контекст наследования незамороженного `Freezable`); эффективная ресурсная поверхность запинена `ThemeResourceSurfaceTests` (STA), CONTEXT.md — «Палитра»; байт-в-байт кроме двух декларированных отклонений, тесты 2632 (2625 + 7), coverage 91.41%
...
**Последнее обновление:** 21.08.2026 — Кандидат 8 обзора №5 (#172, тикет #173, PR #175): темы — одна структура стилей + две палитры вместо двух словарей по 464 строки. Тема разделена на структуру и палитры: новый `ThemeStructure.xaml` несёт общую структуру всех 15 стилей (4 именованных + типовой TabItem + 10 implicit) без единого цвета — кисти в стилях берутся из палитры через `DynamicResource` (механизм живой подмены, прецедент фикса #123); `DarkTheme.xaml`/`LightTheme.xaml` стали палитрами (по 22 цвета + 22 кисти, кисти ссылаются на цвета своей палитры; имена файлов сохранены — механика смены темы `IThemeService`/`ThemeDictionaryManager` и её тесты остались нулевой правкой); файлы палитр различаются только значениями цветов. Класс дефекта «расхождение тем» устранён структурно: структура стилей существует в одном экземпляре. Мёртвые ресурсные ключи `MaterialFlatButton`/`MaterialFlatToggleButton` удалены (0 потребителей). Декларированные отклонения: (1) живая смена темы из Настроек — после подтверждения диалога `MainViewModel.OpenSettings` применяет сохранённую тему через `IThemeService.SetTheme` (фикс скрытого дефекта: раньше — только при следующем запуске); (2) кисти живут в палитрах, а не в структуре — разделяемый незамороженный `Freezable` имеет единственный контекст наследования WPF (смоук чисто-цветовой формы показал частичное перекрашивание), форма «палитра = цвета + кисти» перекрасила весь интерфейс живо в обе стороны. Наблюдаемый контракт сохранён: эффективный набор ключей и значения обеих тем те же, что до перестройки — новый `ThemeResourceSurfaceTests` (STA: точные значения всех цветов и кистей, состав 22+22, структура — только стили, подмена палитры реальной механикой `FindThemeDictionary`, композиция App.xaml). CONTEXT.md — статья «Палитра». Поведение байт-в-байт, кроме двух декларированных отклонений. Тесты 2632 (2625 + 7: 5 пинов поверхности + 2 живой смены из настроек; распределение: приложение 2051 + Document.Tests 454 + Sheets.Tests 127); coverage 91.41%
