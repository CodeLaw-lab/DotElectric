...
### Changed
- Кандидат 8 обзора №5 — темы: одна структура стилей + две палитры вместо двух словарей по 464 строки (#172, тикет #173, PR #175): тема разделена на структуру и палитры — новый `ThemeStructure.xaml` несёт общую структуру всех 15 стилей (4 именованных + типовой TabItem + 10 implicit) без единого цвета, кисти в стилях берутся из палитры через `DynamicResource` (механизм живой подмены, прецедент фикса #123); `DarkTheme.xaml`/`LightTheme.xaml` стали палитрами (по 22 цвета + 22 кисти, кисти ссылаются на цвета своей палитры; имена файлов сохранены — механика смены темы `IThemeService`/`ThemeDictionaryManager` и её тесты остались нулевой правкой, файлы различаются только значениями цветов); класс дефекта «расхождение тем» (структурная правка повторяется в двух местах) устранён структурно; мёртвые ресурсные ключи `MaterialFlatButton`/`MaterialFlatToggleButton` удалены (0 потребителей); единственная структурная правка стиля — в одном файле (цветовой литерал триггера кнопки закрытия вкладки в шаблоне TabItem → `DynamicResource TabCloseButtonHoverBrush`); декларированные отклонения: (1) живая смена темы из Настроек — после подтверждения диалога `MainViewModel.OpenSettings` применяет сохранённую тему через существующий `IThemeService.SetTheme` (фикс скрытого дефекта: раньше тема из настроек применялась только при следующем запуске), (2) кисти живут в палитрах, а не в структуре — разделяемый незамороженный `Freezable` имеет единственный контекст наследования WPF, `DynamicResource`-цвет обновлялся у одного владельца (смоук чисто-цветовой формы показал частичное перекрашивание), форма «палитра = цвета + кисти» перекрасила весь интерфейс живо в обе стороны; наблюдаемый контракт сохранён — эффективный набор ключей и значения обеих тем те же, что до перестройки: новый `ThemeResourceSurfaceTests` (STA, +5 пинов: точные значения всех цветов и кистей обеих палитр, состав 22 цвета + 22 кисти, структура — только стили, подмена палитры реальной механикой `FindThemeDictionary` — цвета следуют, композиция App.xaml); CONTEXT.md — статья «Палитра»; поведение байт-в-байт, кроме двух декларированных отклонений; тесты 2632 (2625 + 7: 5 пинов поверхности + 2 живой смены из настроек; распределение: приложение 2051 + Document.Tests 454 + Sheets.Tests 127), line-rate 91.41%
- Кандидат 7 обзора №5 — MainViewModel без утечки View и строкового протокола ориентаций (#167, тикет #168, PR #170): предпросмотр печати идёт через существующий шов диалогов — тонкая `PrintPreviewViewModel` (документ печати + имя вкладки) + одна строка маппинга в определителе «модель → окно» `WpfDialogHostService`, окно принимает модель через конструктор, интерфейс `IDialogHostService` не растёт; `MainViewModel.PreviewPrint` больше не создаёт окно — импорт слоя Views и статический доступ к приложению удалены из модели представлений; владелец диалогов по умолчанию — главное окно (инъектируемый провайдер в WPF-реализации шва, прецедент `WpfDispatcherService`; дефолт диспетчеробезопасен — getter `MainWindow` потоко-аффинно, живой дефект пойман тестами), объявленное в XAML CenterOwner начинает работать у всех трёх окон (фикс скрытого дефекта); пункты меню «Файл > Новый шаблон» несут типизированную ориентацию — `NewSheetOrientationEntry(Header, Format, Orientation)`, строка-суффикс исчезает, команда принимает пункт целиком (один сеттер XAML, фикс #123 не тронут); фасад — один `CreateNewTab(format, orientation, lastUsedFormat, lastUsedOrientation)` с побайтной цепочкой запасных значений; `ParseSheetFormat` удалён целиком вместе с устаревшими тестами; `/1000` в заголовках меню — через `Coordinate.FormatMm` (вывод побайтно идентичен); схема settings.json не меняется; XML-доки приведены к коду; CONTEXT.md — статья «Предпросмотр печати»; поведение байт-в-байт, кроме декларированного отклонения (центрирование Настроек и диалога произвольного размера на главном окне); тесты 2625 (2655 − 33 теста парсера + 3 пина; распределение: приложение 2044 + Document.Tests 454 + Sheets.Tests 127), line-rate 92.05%
- Кандидат 6 обзора №5 — запасной шрифт: рендер и метрики сходятся в каталоге шрифтов (#162, тикет #163, PR #165): неизвестное имя шрифта ведёт себя как шрифт по умолчанию («ГОСТ А») — одинаково в рендере и в геометрии; статический `FontCatalog` + sealed record `FontDescriptor` в документной библиотеке (прецеденты `SheetFormatCatalog`/`ObjectTypeCatalog`, без DI) — единственный владелец строковой идентичности шрифтов, внутренних имён файлов шрифтов и запасных коэффициентов метрик; `FallbackFontMetrics` делегирует каталогу целиком (собственных констант не осталось), `WpfFontMetrics` берёт из каталога внутренние имена для загрузки и запасные значения (неизвестное имя нормализуется к дефолтному — измеренные значения дефолта, иначе запасные; защитные исключения на null сохранены), `RenderRules` строит pack-URI по внутреннему имени из каталога и для неизвестного имени и null отдаёт шрифт по умолчанию (экземпляры `FontFamily` кэшированы; `FontFamily` в WPF не наследует `Freezable` — заморозка невозможна, кэш неизменяемых экземпляров покрывает цель, задокументированная девиация); литерал «ГОСТ А» в конструкторе `Text` заменён единым источником дефолтов; раунд-трип неизвестного имени байт-совместим (новый пин); ADR-0003 правлен по месту (эмбиентный слот сохраняется; «запасные значения байт-в-байт в двух местах» заменено на «источник запасных значений — один каталог»); статья «Метрики шрифта» в CONTEXT.md дополнена правилом неизвестного имени; известные шрифты — байт-в-байт; декларированные отклонения только для неизвестного имени: рендер «ГОСТ А» вместо Segoe UI и коэффициент ширины 0.5 вместо 0.6 (запасная высота 1.0 не меняется; в production неизвестное имя получает измеренные метрики «ГОСТ А»); XAML, конвертеры, модели представления и сервисы приложения — ноль изменений; проверка документа, контракт метрик и устройство слота не менялись; тесты 2655 (2617 + 38 пинов: FontCatalogTests 25, FallbackFontMetricsTests 10, кэш-пины RenderRules 2, раунд-трип-пин 1; распределение: приложение 2074 + Document.Tests 454 + Sheets.Tests 127), line-rate 91.16%
- Кандидат 2 обзора №5 — формат и проверка знают тип: каталог типов объекта (#157, тикет #158, PR #160): документная библиотека знает свои типы в одной точке — статический `ObjectTypeCatalog` + sealed `ObjectTypeDescriptor` (прецедент `SheetFormatCatalog`, без DI): на тип — строка идентичности, фабрика «запись → модель», действие «модель → запись» и объектные правила проверки (V-003 → V-004 → V-007 → V-005 внутри объекта; неприменимые к типу пропускаются — у текста нет V-007); каталог — единственный владелец строк идентичности «Line»/«Rectangle»/«Text» (ADR-0004, прецедент ADR-0002); `TemplateService` делегирует каталогу чтение и запись объектов (ноль switch'ей по типу; дефолты загрузки отсутствующих полей из единого источника, семантика не изменена); фикс round-trip идентификатора: загрузка сохраняет идентификатор из файла, отсутствующий или пустой → новый (формат существующего генератора), `Clone()` не меняется — копия получает новый идентификатор; объектные правила валидатора (координаты V-003, размеры V-004, цвета V-005, типы линий V-007) ушли в дескрипторы (лист и сервис проверки цвета — аргументы вызова от валидатора), ошибки группируются по объекту (декларированное отклонение); на уровне шаблона остались V-000, V-001, V-006, автор метаданных V-002 и кросс-объектное правило ключей V-002 — один узкий типовой фильтр вместо четырёх каскадов; мёртвый `ValidateObject` удалён из `ITemplateValidator` и `TemplateValidator` — контракт 2 → 1 член (6 тестов удалены, непокрытые сценарии перепокрыты через `Validate`); семантика неизвестного типа сохранена побайтно (неизвестная строка при загрузке — молчаливый пропуск; неизвестный подтип модели при записи — запись без типа, только идентификатор; неизвестный подтип — пусто во всех объектных правилах); плоская структура записи объекта и XML-атрибуты без изменений; модель (`TemplateObjectBase`) не изменена вовсе — хуков сериализации и проверки на базе объектов нет; приложение и XAML — ноль изменений; поведение байт-в-байт, кроме двух декларированных отклонений (round-trip идентификатора; группировка ошибок по объекту в диалоге сохранения); тесты 2617 (2603 − 6 тестов удалённого шва + 20 пинов; распределение: приложение 2072 + Document.Tests 418 + Sheets.Tests 127), line-rate 91.18%
- Кандидат 5 обзора №5 — deletion sweep №2: мёртвое знание приложения (#152, тикет #153, PR #155): мёртвое и расходящееся знание удалено одним проходом — `TextToolSettings` удалён (четыре дефолта инлайнены в `TextTool` — шрифт/размер из библиотечного `DocumentDefaults`, тип `TextType.Text`, содержимое — локальная константа «Текст»; осиротевшие сеттеры `SetTextType`/`SetFontSize`/`SetDefaultContent` удалены с 5 тестами); класс `Grid` удалён — шаг сетки живёт одной константой `EditorSettings.DefaultGridStepMicrons` вместо трёх копий (`GridSettings.FromDefaultGrid` переименован в `CreateDefault`); настройка-призрак `DefaultZoom` удалена целиком (AppSettings, SettingsViewModel + `ZoomOptions`, ComboBox «Масштаб:» в SettingsView.xaml, тесты; схема settings.json байт-совместима — неизвестные ключи игнорируются); мёртвые константы `DoubleClickThresholdMs`/`DefaultSheetOffsetMm` удалены, `NudgeStepMicrons` переиспользован со значением 100 и подключён в `EditorViewModel.NudgeStep` (расхождение «константа 1 мм vs хардкод 0.1 мм» устранено, поведение сохранено); метки ориентации «кн.»/«алб.» — одна статическая точка `Helpers/OrientationLabels.For` (три делегата, неизвестная ориентация — явный throw); Rotate ±90 — приватное ядро со знаком как параметром, команды — тонкие делегаты; mojibake восстановлен (селективное декодирование CP1251→UTF-8, 86 строк: `AutosaveService` — 11 строковых литералов логов + доки/комментарии, `MainViewModel`, `IMessageBoxProvider`); `SnapHelper.SnapObject` удалён (0 потребителей) с 3 тестами; три теста-реликта `ValidationService_ValidateObject*` перенесены в `Document.Tests/TemplateValidatorTests` (assert'ы побайтно); поведение байт-в-байт (единственное видимое изменение — восстановленные строки логов); тесты 2603 (2613 − 10 тестов удалённого кода), line-rate 91.13% (без изменений)
- Кандидат 4 обзора №5 — добить миграцию: тесты библиотек застряли в приложении (#147, тикет #148, PR #150): тесты библиотечных типов перенесены из `DotElectric.TemplateEditor.Tests` в родные проекты библиотек — V-правила тремя источниками (`ValidationServiceTests` 48 — имя-реликт устранено: app-`ValidationService` там не тестировалась, `AdditionalValidationServiceTests` 10, `ExtendedValidationServiceTests` 17) слиты в один `Document.Tests/TemplateValidatorTests.cs` + `HexColorValidationTests.cs` (4); новые `TemplateTests.cs` (11), `MetadataTests.cs` (2), `RectMicronsTests.cs` (16: 9 по карте спеки + 7 из секции RectMicrons `SelectionBoxHelperTests` — по deletion-чеклисту спеки) в `Document.Tests` и `SheetTests.cs` (10) в `Sheets.Tests`; влито в существующие `CoordinateTests` (+8), `PointMicronsTests` (+7), `TemplateServiceTests` (+4); четыре локальные фабрики шаблона переехавших тестов слиты в общую фикстуру `TestTemplates`; app-остатки смешанных файлов влиты в родительские (конвенция R4.4), `Models/TemplateTests.cs` переименован в `GridSettingsTests.cs`; 5 файлов-источников и 3 реликтовых отчёта удалены; `FixedFontMetrics`/`FontMetricsTestCollection` ×2 — осознанные копии (коллекция xUnit обязана жить в сборке тестов); ни один тест не добавлен и не удалён — 2613 (распределение: приложение 2085 + Document.Tests 401 + Sheets.Tests 127), production-код и XAML — ноль изменений (чистый перенос), line-rate 91.13% (без изменений)
- Кандидат 3 обзора №5 — один шов проверки документа (#142, тикет #143, PR #145): проверка документа выставлена одним швом — обёртка `ITemplateService.Validate(Template) → IEnumerable<string>` (побайтный дубль `ValidationError.ToString()`, спрятанный фильтр серьёзности, мёртвая null-ветка) удалена из интерфейса (5 → 4 члена) и из `TemplateService` (метод, поле, параметр конструктора, запасное создание); единственный потребитель `TabOperationsService` инжектит `ITemplateValidator` напрямую — фильтр «в диалог только `Error`» и склейка сообщений (`ToString()` через `\n`) в потребителе побайтно; цепочка цвета сокращена: поле-посредник `ValidationService.Default` и обёртка `ValidationService.ValidateHexColor` удалены (DI регистрирует `HexColorValidation.Default` напрямую, три панели свойств вызывают `HexColorValidation.Validate`), `IValidationService` сохранён (шов конструктора `TemplateValidator`); документация приведена к поведению (XML-комментарий `ValidationSeverity.Error`, статья «Правила проверки документа» в CONTEXT.md); 6 тестов удалённой обёртки удалены (включая единственный пропуск набора), путь сохранения на моке валидатора + пин-тест «предупреждения не показывают диалог»; поведение байт-в-байт (XAML — 0 изменений); тесты 2613 (0 пропусков), line-rate 91.13%
- Кандидат 1 обзора №5 — документ без редактора (#137, тикет #138, PR #140): документная библиотека `DotElectric.Document` перестала нести знание редактора — `PhysicalConstants` переименован в `DocumentConstants` и несёт только `LineHitToleranceMicrons` (единственная константа, которую читает сам документ — попадание в тело линии/прямоугольника); шесть констант взаимодействия (`HandleHitToleranceMicrons`, `SelectionBoxThresholdMicrons`, `MinResizeSizeMicrons`, `MinFontSizeMicrons`, `MinDimensionMicrons`, `MaxCustomSheetSizeMm`) перенесены в `EditorSettings` (секция Interaction, имена и значения сохранены); привязка к сетке удалена из библиотеки целиком (`Coordinate.SnapToGrid`, `PointMicrons.SnapToGrid`) — формула живёт приватным скалярным ядром в `SnapHelper` (публичная поверхность без изменений, семантика исключения побайтно; CONTEXT.md: «Сетка — не понятие документа»); дубль `MicronsPerMm` устранён (потребители на `Coordinate.MicronsPerMm`, включая хардкод слоя узлов сетки); комментарий `Template.Sheet` без имени потребителя из приложения; 6 тестов формулы переехали из `DotElectric.Document.Tests` в `SnapHelperTests`; поведение байт-в-байт (XAML — 0 изменений); тесты 2617 (базовое число ветки миграции — без изменений), line-rate 91.11%
- Кандидат 6 обзора №4 — TextGeometry: геометрия повёрнутого текста из модели (#118, тикет #119, PR #121): WPF-знание о геометрии повёрнутого текста (~60% класса `Text`) вынесено в статический модуль `TextGeometry` (Helpers) — `LayoutOffset` (смещение WPF LayoutTransform, публичная — тестируется напрямую), `Corner` (углы 0–3 в порядке каталога MarkerLayout, индекс вне диапазона — явный throw), `Contains` (обратное вращение вокруг фактического центра вращения), `BoundingBox`; модель `Text` — тонкие делегации (8 свойств повёрнутых углов для XAML маркеров + полиморфные contains/bounding box; INPC-проводка на модели; 393 → 217 строк); мёртвые швы удалены (`VisualLeft/Right/Bottom/Top` ×4 с INPC-обвязкой — 0 потребителей, `RotationAngleValid` — тавтология, +2 тавтологичных теста); 12 геометрических тестов мигрированы в `TextGeometryTests` (offset-тесты — прямым вызовом `LayoutOffset`), `RotatedCorners_AllLieOnBoundingBoxEdges` остался пином XAML-поверхности; print-дефект обзора («повёрнутый текст смещён в print preview») проверен на коде и не подтвердился — печать и канвас компенсируются WPF-раскладкой одинаково (фикс кандидата 1); XAML и все потребители (MarkerLayout, HitTestHelper, SelectionBoxHelper, PrintDocumentGenerator, PreviewLineChangedBehavior) — 0 изменений; поведение байт-в-байт; тесты 2630 (−2 тавтологичных −12 мигрировавших +13 методов модуля), line-rate 90.34%
- Кандидат 5 обзора №4 — форматы листа: один каталог (#113, тикет #114, PR #116): знание о стандартных форматах (11 точек: два switch'а в `Sheet`, HashSet валидатора, список настроек, 20 MenuItem с размерами в заголовках, 5 кнопок диалога, парсер суффиксов, дефолт «A3» ×6) собрано в статический `SheetFormatCatalog` (Models) + sealed record `SheetFormat` — единственный источник фиксированного набора (10 форматов: A0–A4 + пять полуформатов); строковая идентичность формата зафиксирована в ADR-0002 (сериализуется в .tdel/settings.json); `Sheet.FromFormat`/`GetDefaultOrientation` делегируют каталогу (switch'и и молчаливый Landscape удалены — единый throw); валидатор: `ValidFormats` удалён → `Contains` + Custom, V-006 перечисляет канонические 10 + Custom без латинских дублей (файлы с `A4X2` по-прежнему валидны); `FormatOptions` настроек из каталога; меню «Файл > Новый шаблон» генерируется из каталога через ItemsSource (заголовки компонуются из размеров, байт-в-байт); мёртвая настройка `DefaultSheetFormat` оживает fallback'ом в цепочке создания вкладки; дефолт «A3» ×6 — через `const DefaultName`; диалог произвольного размера — TryGet-no-op вместо глотания исключений; CONTEXT.md: «Стандартный формат» и «Пользовательский формат»; поведение байт-в-байт; тесты 2630 (+54), line-rate 90.38%
- Кандидат 8 обзора №4 — мёртвые швы: deletion sweep (#108, тикет #109, PR #111): весь код с нулём production-потребителей удалён вместе с тестами — callback-параметры конструктора `ZoomPanManager` (wiring сетки — только `SetGridRefreshCallback`, мёртвый вызов zoom-callback удалён); мёртвое поле `_templateService` и параметр `ITemplateService` обоих конструкторов `EditorViewModel` (фабрика не резолвит сервис шаблонов), `_gridNodeGenerator` — локальная переменная конструктора; `ITextToolSettings` удалён целиком (DI-регистрация, интерфейс, опциональный параметр `TextTool` — инструмент создаёт настройки напрямую); DI-регистрация `IFontMetrics` удалена (интерфейс жив тестовыми моками, production — статический `FontMetrics.Default`); 4 конвертера без биндингов удалены (`ZoomToStringConverter`, `LineTypeToStringConverter`, `TextTypeToStringConverter`, `RelativeMicronsToPixelConverter`); ресурсы всех конвертеров консолидированы в корневом словаре ресурсов приложения — каждый класс объявлен ровно один раз (исключение — view-локальный `InverseBooleanConverter` в SettingsView: STA-тесты без ресурсов приложения); `ValidationService.ValidateRotation` удалён (0 точек вызова); контракт Автосохранения типизирован (`Template` вместо `object`, мёртвый сеттер `FilePath` удалён из `IAutosaveTab`), горизонт очистки — константа `AutosaveCleanupDays`; механизм Автосохранения сохранён (решение Q5-b), read-поверхность оставлена до продуктовой спеки восстановления; `EnumToIndexConverter` sealed; поведение байт-в-байт; термин «Автосохранение» в CONTEXT.md; тесты 2576 (−38 тестов удалённого кода), line-rate 90.6%
- Кандидат 4 обзора №4 — раскладка маркеров: один модуль (#103, тикет #104, PR #106): знание «какие маркеры выделения есть у объекта и где они» собрано в один статический модуль `MarkerLayout` (Helpers) — каталог маркеров по типу в порядке приоритета hit-проверки, позиции в модельных координатах, hit-тест с допуском `min(8 мм, minDim/3)` (семантика фикса #82), классификация (угол/ребро/конец линии), курсорная политика; `HitTestHelper` сжат до хита по телу (зоны/допуск/per-type методы + 6 мёртвых публичных методов удалены целиком); `ResizeMath` — чистая математика drag'а (курсорные функции и 4 набора классификации переехали в модуль); `ResizeTool` лишился 8 ручных снапшот-полей (единственный модельный `ResizeState`); `IEditorContext` 26 → 25 членов (`HoveredHandle` — приватное поле SelectTool); 4 silent-default ветки → явные throw; enum `ResizeHandle` определён единственно в MarkerLayout; поведение байт-в-байт, XAML не менялся; термин «Маркер выделения» в CONTEXT.md; тесты 2614, line-rate 90.25%
...
### Fixed
- Тёмная тема: сгенерированное подменю «Файл > Новый шаблон» отображалось светлым, текст сливался с фоном (#123) — явные стили контейнеров, добавленные при переводе меню на генерацию из каталога (#116), подавляли implicit-стиль MenuItem из темы; в оба стиля контейнеров добавлены DynamicResource-сеттеры Background/Foreground + Padding (паттерн корневого Menu), живое переключение темы F9 сохранено; STA-тест — структурный пин темизации обоих стилей; светлая тема байт-в-байт
