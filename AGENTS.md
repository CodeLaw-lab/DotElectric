...
| **Coverage** | **76.4% line-rate** | **90.13% line-rate (gate 80% достигнут)** |
| **CI coverage gate** | **75%** | **80% (факт 90.13%)** |
...
**Tests:** 2691 total (2690 passed, 1 pre-existing skip)
...
- **Tools:** ITool (без Name) + IEditorContext (27 членов, типизированные `PushTool`/`PopTool`/`ActivateTool`, без WPF ICommand; рамка выделения — методы `SetSelectionBox`/`ClearSelectionBox`; preview — член `PreviewManager`; пан — жест роутера, не член интерфейса) + `ToolKind` + ResizeMath (чистые функции) + ShortcutRegistry
