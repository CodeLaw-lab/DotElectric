...
| **Формат и проверка знают тип (каталог типов объекта)** | **6 ручных точек знания типа в документной библиотеке: ветвь чтения записи, ветвь записи и четыре каскада «is Line / is Rectangle / is Text» в правилах проверки; идентификатор перегенерировался на каждый цикл «сохранил → загрузил», хотя сериализация его писала; мёртвый второй член контракта валидации (`ValidateObject`, 0 потребителей в production)** | **один каталог `ObjectTypeCatalog` + sealed `ObjectTypeDescriptor` (DotElectric.Document, прецедент SheetFormatCatalog, без DI): строка идентичности типа, фабрика «запись → модель», действие «модель → запись», объектные правила (V-003 → V-004 → V-007 → V-005 внутри объекта); сервис шаблона — ноль switch'ей по типу; идентификатор стабилен через round-trip (отсутствующий/пустой → новый, копия получает новый); валидатор — объектные правила через дескрипторы с группировкой ошибок по объекту, один типовой фильтр на уровне шаблона (V-002 ключи); контракт валидации 2 → 1 член; строковая идентичность и стабильный идентификатор — ADR-0004; модель не изменена; приложение/XAML — 0 изменений; тесты 2603 → 2617 (−6 удалённого шва +20 пинов), покрытие 91.18%** |
| **Запасной шрифт (каталог шрифтов)** | **два ответа на один вопрос: неизвестное имя рендерилось Segoe UI, а геометрия (маркеры, hit-тест, рамка, ресайз, печать) считалась по константам 1.0/0.6; дубль запасных констант в двух сборках (ADR-0003 объявлял дубль осознанным); внутренние имена файлов шрифтов в двух местах; `FontFamily` аллоцировался на каждый вызов биндинга** | **неизвестный шрифт = шрифт по умолчанию («ГОСТ А») в рендере и метриках: статический `FontCatalog` + sealed `FontDescriptor` (DotElectric.Document, прецеденты SheetFormatCatalog/ObjectTypeCatalog, без DI) — единственный владелец идентичности, внутренних имён и запасных коэффициентов; `FallbackFontMetrics` делегирует целиком, `WpfFontMetrics` и `RenderRules` — потребители (pack-URI по внутреннему имени, кэш `FontFamily`); раунд-трип неизвестного имени байт-совместим; ADR-0003 правлен по месту; «Метрики шрифта» в CONTEXT.md дополнены; известные шрифты байт-в-байт, два декларированных отклонения для неизвестного имени (рендер «ГОСТ А» вместо Segoe UI, ширина 0.5 вместо 0.6); приложение/XAML — 0 изменений; тесты 2617 → 2655 (+38 пинов), покрытие 91.16%** |
| **Предпросмотр печати и ориентации меню (кандидат 7 обзора №5)** | **утечка View: `PreviewPrint` сам создавал `PrintPreviewWindow` в обход шва диалогов (`Application.Current.MainWindow` в модели представлений, единственный импорт Views); ориентации — строковый протокол «A4L»/«A4P» с парсингом последнего символа в фасаде; `/1000` в заголовках меню вне помощников; CenterOwner объявлен, но владелец ставился только у предпросмотра** | **предпросмотр печати через `IDialogHostService`: `PrintPreviewViewModel` (документ + имя вкладки) + строка маппинга, окно с конструктором от модели; владелец по умолчанию — главное окно (инъектируемый провайдер в WPF-реализации шва, дефолт диспетчеробезопасен — getter `MainWindow` потоко-аффинно, дефект пойман тестами), CenterOwner работает у всех трёх окон; типизированная ориентация меню (`NewSheetOrientationEntry(Header, Format, Orientation)`), команда принимает пункт целиком; фасад `CreateNewTab` с типизированной ориентацией, цепочки запасных значений побайтно; парсер суффиксов удалён целиком; `Coordinate.FormatMm` в заголовках; settings.json байт-совместим; «Предпросмотр печати» в CONTEXT.md; байт-в-байт кроме центрирования двух диалогов; тесты 2655 → 2625 (−33 парсера, +3 пина), покрытие 92.05%** |

**Build:** 0 errors, 0 warnings
**Tests:** 2625 total (2625 passed, 0 пропусков)
...
- **ObjectTypeCatalog:** статический каталог дескрипторов типов объекта (DotElectric.Document, прецедент SheetFormatCatalog) + sealed `ObjectTypeDescriptor` (TypeName, ModelType, FromDto, WriteToDto, Validate) — единственный владелец строк идентичности «Line»/«Rectangle»/«Text» (ADR-0004); ровно три записи; TemplateService читает и пишет объекты только через каталог (ноль switch'ей по типу); объектные правила проверки (V-003 → V-004 → V-007 → V-005) — в дескрипторах (лист и сервис проверки цвета — аргументы вызова от валидатора); идентичность точная (регистрозависимая); неизвестная строка типа при загрузке — молчаливый пропуск; неизвестный подтип модели при записи — запись без типа (только идентификатор), без объектных правил при проверке; идентификатор объекта стабилен через «сохранил → загрузил» (отсутствующий/пустой → новый), копия получает новый
...
- **Validation:** `ITemplateValidator`/`TemplateValidator` (domain; контракт — единственный член `Validate(Template)` после удаления мёртвого `ValidateObject` (кандидат 2 обзора №5); правила уровня шаблона — в валидаторе, объектные правила — в дескрипторах каталога типов; ошибки группируются по объекту) + `ValidationService` (UI)
...
85. WPF-статика потоко-аффинна: getter `Application.Current.MainWindow` (и члены `DispatcherObject` вообще) кидает `InvalidOperationException` с чужого потока — в production вызов идёт с потока диспетчера, но STA-тесты живут на собственных потоках. Не читай WPF-статику напрямую в сервисах: инъектируй провайдер опциональным параметром конструктора (прецеденты `WpfDispatcherService` с `Dispatcher?`, `WpfDialogHostService` с `Func<Window?>`), дефолт guarded через `Dispatcher.CheckAccess()`. Дефект пойман тестами в кандидате 7 обзора №5.
...
## Sprint — Кандидат 2 обзора №5: формат и проверка знают тип — каталог типов объекта (#157–#158, PR #160, 21.08.2026)

### Проблема
Кандидат 2 архитектурного обзора №5 (отчёт `architecture-review-20260820-1540.html`) — «Формат и проверка знают тип: полиморф вместо шести ручных точек» (Сильная рекомендация). Модель документа полиморфна (абстрактные `ContainsPoint`/`GetBoundingBox`/`CaptureResizeState`/`ApplyResize`/`Clone`/`Move`), но сериализация и проверка документа — нет: добавление нового типа объекта (круг/дуга — штатный сценарий wayfinder-карты #125) требовало ручных несинхронизируемых правок в шести точках библиотеки (ветвь чтения записи, ветвь записи, четыре каскада «is Line / is Rectangle / is Text» в правилах проверки). Формат писал идентификатор объекта (`<Id>`), но маппинг его выбрасывал — идентификаторы перегенерировались на каждый цикл «сохранил → загрузил». Параллельно библиотека несла мёртвый шов: второй член контракта валидации (`ValidateObject`) при нуле production-потребителей. Grilling-сессия 20–21.08.2026 (Q1–Q16): решение wayfinder #131-Q2 («новых механизмов расширения не вводить») пересмотрено — чеклист расширяемости и есть эти ручные точки; #131-Q3 (молчаливый пропуск неизвестного типа) и #131-Q4 (явный throw точек редактора) сохранены.

### Решение — атомарный флип (TDD на новом шве, один PR)
- **`ObjectTypeCatalog` + `ObjectTypeDescriptor`** (DotElectric.Document, статический, без DI — прецедент `SheetFormatCatalog`): ровно три дескриптора (Line/Rectangle/Text), каждый несёт строку идентичности типа, фабрику «запись → модель», действие «модель → запись» и объектные правила проверки (V-003 → V-004 → V-007 → V-005 внутри объекта; неприменимые к типу пропускаются — у текста нет V-007). Идентичность точная (`StringComparer.Ordinal` — побайтно со старым string-switch): неизвестная строка → пропуск. Модель (`TemplateObjectBase`) не изменена вовсе — хуков сериализации и проверки на базе объектов нет; зависимости проверки (лист, сервис проверки цвета) передаются дескриптору аргументами вызова от валидатора.
- **TemplateService — ноль switch'ей по типу:** чтение и запись объектов делегируют каталогу; дефолты загрузки отсутствующих полей сохранены побайтно (`DocumentDefaults` + zero-литералы). Фикс round-trip идентификатора через init-only `Id` в object initializer фабрик: идентификатор из файла сохраняется; отсутствующий или пустой → новый (`Guid.NewGuid().ToString()` — формат существующего генератора); `Clone()` не меняется — копия получает новый идентификатор.
- **Валидатор:** объектные правила (координаты V-003, размеры V-004, цвета V-005, типы линий V-007) ушли в дескрипторы; ошибки группируются по объекту (декларированное отклонение). На уровне шаблона в нынешнем порядке остались: V-000, null-лист V-006, формат листа V-006, автор метаданных V-002, идентификаторы V-001 и кросс-объектное правило дублирующихся ключей V-002 — единственный оставшийся типовой фильтр (is-Text) вместо четырёх каскадов. Семантика правила координат побайтно: линия — до двух раздельных ошибок по концам; прямоугольник — опорная точка и правый верхний угол (без проверки отрицательности правого/верхнего); текст — только опорная точка. Публичная сигнатура `Validate(Template)` без изменений.
- **Мёртвый шов удалён:** `ValidateObject` удалён из `ITemplateValidator` и `TemplateValidator` — контракт 2 → 1 член; 6 тестов удалены: сценарии null-листа и валидного объекта уже покрыты через `Validate`, прямоугольник вне листа (V-003) и нулевой ширины (V-004) перепокрыты через `Validate`, null-объект и валидный тип линии удалены вместе с швом / покрыты существующими.
- **Семантика неизвестного типа сохранена побайтно:** неизвестная строка типа при загрузке — молчаливый пропуск; неизвестный подтип модели при записи — запись без типа (только идентификатор), исчезающая при перечитывании; неизвестный подтип — пусто во всех объектных правилах проверки (инвариант запинен).
- **ADR-0004** «Идентичность объекта в формате .tdel»: идентичность типа — строка, единственный владелец — каталог (прецедент ADR-0002); идентификатор стабилен через «сохранил → загрузил», копия получает новый.
- **Плоская структура записи объекта не реструктурирована:** XML байт-в-байт на запись, включая «чужие» zero-поля; XML-атрибуты без изменений. Приложение и XAML — ноль изменений.
- **Тесты:** TDD red-фаза (round-trip идентификатора и группировка ошибок падали на текущем коде → green после переподключения); новые пины только в `Document.Tests` (тесты едут с кодом), дескрипторы тестируются через швы (Load/Save, `Validate`, поверхность каталога): round-trip идентификатора (сохраняется на смешанном шаблоне трёх типов / отсутствует → новый / пустой → новый); неизвестный подтип на записи — запись без типа, идентификатор пишется, при загрузке пропускается; группировка ошибок по объекту (точная последовательность RuleId+ObjectId); идентичность каталога (ровно три записи, точная идентичность, неизвестная строка → пропуск, неизвестный подтип → нет дескриптора); инвариант неизвестного подтипа. Существующие round-trip-тесты и тесты V-правил зелёные без модификаций — байт-совместимость доказана.

### Итоги
- Поведение байт-в-байт, кроме двух декларированных отклонений: round-trip идентификатора и группировка ошибок по объекту в диалоге сохранения. Библиотечная часть wayfinder-чеклиста #131 замещена одной точкой в каталоге типов; редакторная часть чеклиста (маркеры, рендеринг, инструменты, панели свойств, XAML, печать, предпросмотр, строка состояния) остаётся в силе — это знание редактора, не документа.
- Тесты: **2617 (2617 passed, 0 пропусков)** = базовая линия 2603 − 6 тестов удалённого `ValidateObject` + 20 новых пинов (распределение: приложение 2072 + `Document.Tests` 418 + `Sheets.Tests` 127).
- Deletion-чеклист: строковые литералы «Line»/«Rectangle»/«Text» в библиотеке — только в `ObjectTypeCatalog` (6 вхождений); в `TemplateService` ноль switch'ей по типу; в валидаторе один типовой фильтр; `ValidateObject` — 0 вхождений в src/; модель/приложение/XAML — ноль изменений.
- Двухосевое code-review: Standards — 0 hard violations (judgment calls закрыты фикс-коммитом: дедупликация V-007 в общий помощник `ValidateLineTypeValue`, предикат выхода за лист `OutsideSheet` на все четыре точки, уточнение XML-дока дескриптора); Spec — полное соответствие (трактовка позиции V-002 ключей задокументирована в теле PR: кросс-объектное правило на уровне шаблона перед группировкой, по grilling Q14).
- CI 5/5 зелёный, merged line-rate **91.18%** (gate 80%). Слияние: PR #160 в develop (merge 0336918); спека #157 и тикет #158 закрылись автоматически.

### Не вошло / отложено (радар)
- Кандидат 6 обзора №5 (запасной шрифт: fallback рендера `RenderRules` не совпадает с `FallbackFontMetrics`) — отдельная спека.
- Кандидаты 7/8 обзора №5 — не начаты.
- Реструктуризация плоской записи объекта («чужие» zero-поля в XML) — осознанно отклонена ради байт-совместимости.
- Оживление `ValidationError.ObjectId` в потребительской цепочке (мёртв в production) — остаётся как есть.

**Build:** 0 errors, 0 warnings
**Tests:** 2617 total (2617 passed, 0 пропусков; распределение: приложение 2072 + Document.Tests 418 + Sheets.Tests 127)
**Coverage:** 91.18% line-rate ✅ (gate 80% достигнут)

## Sprint — Кандидат 6 обзора №5: запасной шрифт — рендер и метрики сходятся в каталоге шрифтов (#162–#163, PR #165, 21.08.2026)

### Проблема
Для текста с неизвестным именем шрифта редактор давал два разных ответа: конвейер отображения (канвас, предпросмотр, печать, встроенный редактор) рендерил Segoe UI, а геометрия (`Text.WidthMicrons`/`HeightMicrons` → маркеры выделения, попадание клика, рамка выделения, ресайз, раскладка печати) считалась по константам 1.0/0.6. Запасные константы продублированы в `FallbackFontMetrics` (библиотека) и запасных ветках `WpfFontMetrics` (приложение) — ADR-0003 объявлял дубль осознанным; внутренние имена файлов шрифтов (`GOST Type AU`/`GOST Type BU`) жили в двух местах; `RenderRules.FontFamilyFor` аллоцировал новый `FontFamily` на каждый вызов биндинга.

### Решение
- **Продуктовое решение**: неизвестный шрифт = шрифт по умолчанию («ГОСТ А») — одинаково в рендере и в метриках (вариант «оставить Segoe UI и измерить его метрики» отклонён: метрики системного шрифта машино-зависимы и непроверяемы вне редактора).
- **Новый модуль — статический каталог шрифтов** `FontCatalog` + sealed record `FontDescriptor` в `DotElectric.Document` (прецеденты `SheetFormatCatalog`/`ObjectTypeCatalog`, без DI): ровно две записи (доменное имя, внутреннее имя файла шрифта, запасные коэффициенты высоты/ширины), дефолт из единого источника дефолтов, нормализация «неизвестное имя → имя по умолчанию». Каталог — единственный владелец строковой идентичности шрифтов, внутренних имён файлов и запасных коэффициентов.
- **Потребители**: `FallbackFontMetrics` делегирует каталогу целиком (собственных констант не осталось); `WpfFontMetrics` — инициализация обходит записи каталога, запасные значения из каталога, неизвестное имя нормализуется к дефолтному (измеренные значения дефолта, иначе запасные), защитные исключения на null сохранены; `RenderRules` — pack-URI по внутреннему имени из каталога (схема pack-URI остаётся знанием приложения), неизвестное имя и null — шрифт по умолчанию, экземпляры `FontFamily` кэшированы. Задокументированная девиация: `FontFamily` в WPF не наследует `Freezable` — заморозка невозможна, кэш неизменяемых экземпляров покрывает цель.
- Литерал «ГОСТ А» в конструкторе `Text` → `DocumentDefaults.DefaultFontName`.
- **Раунд-трип байт-совместим**: неизвестное имя переживает «сохранил → загрузил» (новый пин).
- **ADR-0003 правлен по месту** (эмбиентный слот сохраняется; «запасные значения в двух местах» заменено на «источник — один каталог»); статья «Метрики шрифта» в CONTEXT.md дополнена правилом неизвестного имени.

### Итоги
- Известные шрифты — байт-в-байт; декларированные отклонения только для неизвестного имени: рендер «ГОСТ А» вместо Segoe UI, коэффициент ширины 0.5 вместо 0.6 (запасная высота 1.0 не меняется; в production неизвестное имя получает измеренные метрики «ГОСТ А» — в этом смысл решения).
- TDD red-фаза; тесты 2617 → 2655 (+38: FontCatalogTests 25, FallbackFontMetricsTests 10, кэш-пины RenderRules 2, раунд-трип-пин 1); слитый line-rate 91.16% (gate 80%).
- Deletion-проверка: запасная константа 0.6 — ноль вхождений в production; Segoe UI в конвейерах рендера — ноль; дубль внутренних имён файлов шрифтов устранён.
- Двухосевое code-review: Standards — 0 hard violations (3 judgement calls закрыты фикс-коммитом + 1 принят: схема pack-URI в двух потребителях app-сборки — перенос в библиотеку запрещён §3.1); Spec — полное соответствие.
- Смоук: старт приложения, новый шаблон, активация инструмента «Текст» — UI живой; клик по канвасу фоновой автоматизацией недоступен (ограничение стека, путь создания текста срезом не менялся).
- XAML, конвертеры, модели представления и сервисы приложения — ноль изменений; проверка документа, контракт метрик и устройство слота не менялись.

**Build:** 0 errors, 0 warnings
**Tests:** 2655 total (2655 passed, 0 пропусков; распределение: приложение 2074 + Document.Tests 454 + Sheets.Tests 127)
**Coverage:** 91.16% line-rate ✅ (gate 80% достигнут)

## Sprint — Кандидат 7 обзора №5: предпросмотр печати через шов диалогов и типизированная ориентация меню (#167–#170, 21.08.2026)

### Проблема
Главная модель представлений нарушала две границы: предпросмотр печати создавал WPF-окно напрямую (`new PrintPreviewWindow(...)`, `window.Owner = System.Windows.Application.Current.MainWindow`) в обход шва диалогов `IDialogHostService` — единственный импорт слоя Views в MainViewModel; ориентации меню «Файл > Новый шаблон» ходили строковым протоколом «A4L»/«A4P» с парсингом последнего символа в `TabOperationsService.ParseSheetFormat` при живом `enum SheetOrientation`. Побочно: все три диалоговых окна объявляют `WindowStartupLocation="CenterOwner"`, но владелец ставился только у предпросмотра; `/1000` в заголовках меню — вне помощников перевода единиц.

### Решение
- **Предпросмотр печати через существующий шов:** новая тонкая `PrintPreviewViewModel` (sealed record: документ печати + имя вкладки) + одна строка маппинга в `WpfDialogHostService.ResolveWindowDescriptor`; `PrintPreviewWindow` принимает модель через конструктор (существующий создатель окон умеет одноаргументный конструктор); интерфейс `IDialogHostService` не растёт. Генерация документа остаётся в MainViewModel; try/catch вокруг генерации и показа — там же.
- **Владелец диалогов по умолчанию — главное окно:** разрешается в WPF-реализации шва; провайдер владельца инъектируемый (`Func<Window?>`, прецедент `WpfDispatcherService`), дефолт диспетчеробезопасен (`Dispatcher.CheckAccess()` — getter `MainWindow` потоко-аффинно; живой дефект пойман тестами в фикс-коммите). Объявленное в XAML центрирование начинает работать у всех трёх окон — декларированное отклонение (фикс скрытого дефекта).
- **Типизированная ориентация меню:** `NewSheetOrientationEntry(Header, Format, Orientation)` — строка-суффикс исчезает; XAML передаёт команде всю запись (один сеттер `CommandParameter`, фикс #123 не тронут); `NewTab` принимает запись. Фасад — один `CreateNewTab(string? format, SheetOrientation? orientation, string? lastUsedFormat, string? lastUsedOrientation)`; цепочки запасных значений побайтно («явная ориентация → `Enum.TryParse` строки настроек → ориентация по умолчанию формата», включая курьёз `?? "Landscape"`); схема settings.json не меняется.
- **Удаления и правки:** `ParseSheetFormat` удалён целиком вместе с устаревшими тестами (reflection-файл + 7 прямых тестов); `/1000` в заголовках → `Coordinate.FormatMm` (вывод побайтно идентичен); XML-доки приведены к коду (суффиксный протокол из дока `NewTab`, «PreviewPrint»/«Print» из доков фасада).
- **CONTEXT.md:** новая статья «Предпросмотр печати» (отличается от «Предпросмотра» — временного контура рисования). ADR не нужен.

### Тесты и верификация
- TDD red-фаза; тесты: 2655 → **2625** (2625 passed, 0 пропусков; распределение: приложение 2044 + Document.Tests 454 + Sheets.Tests 127) = −33 теста удалённого парсера + 3 новых пина (маппинг и конструктор окна предпросмотра, владелец через провайдер); усилены пины швов (идентичность документа в показанной модели — STA, аргументы фасада для пустого пункта меню); ~60 вызовов команды создания вкладки мигрированы на локальные помощники (конвенция per-file).
- Deletion-проверка: `ParseSheetFormat` — 0 вхождений в src/; `Application.Current` и импорт Views в MainViewModel — 0; строки-суффиксы в модели меню — 0.
- Сборка 0 ошибок / 0 предупреждений; слитый line-rate методом CI (ReportGenerator) **92.05%** (gate 80%).
- Двухосевое code-review: Standards — 0 hard violations (judgment calls задокументированы: `FixedDocument` в record — существующий санкционированный payload; 4 nullable-параметра фасада — решение grilling; per-file хелперы — конвенция); Spec — 3 MINOR закрыты фикс-коммитом (усилены пины швов 1 и 5, покрыта ветка владельца).
- Смоук: старт приложения; вкладка из тулбара (последний формат из настроек); «Файл > Предпросмотр печати» открывает окно «Предпросмотр печати — A0 (кн.) — Без имени», центрированное на главном окне.

### Итоги
Поведение байт-в-байт, кроме декларированного отклонения (центрирование Настроек и диалога произвольного размера на главном окне). XAML — один сеттер; документная библиотека не менялась. Слияние: PR #170 в develop (merge da5577e); спека #167 и тикет #168 закрылись автоматически.
