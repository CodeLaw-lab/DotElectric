...
| **Coverage** | **76.4% line-rate** | **90.08% line-rate (gate 80% достигнут)** |
| **CI coverage gate** | **75%** | **80% (факт 90.08%)** |
...
**Tests:** 2674 total (2673 passed, 1 pre-existing skip)
...
- **Tools:** ITool (без Name) + IEditorContext (26 членов, типизированные `PushTool`/`PopTool`/`ActivateTool`, без WPF ICommand; рамка выделения — методы `SetSelectionBox`/`ClearSelectionBox`; preview — член `PreviewManager`; пан — жест роутера, не член интерфейса; грязность — не член интерфейса) + `ToolKind` + ResizeMath (чистые функции) + ShortcutRegistry
