...
**Common Mistakes (new):**
85. WPF-статика потоко-аффинна: getter `Application.Current.MainWindow` (и члены `DispatcherObject` вообще) кидает `InvalidOperationException` с чужого потока — в production вызов идёт с потока диспетчера, но STA-тесты живут на собственных потоках. Не читай WPF-статику напрямую в сервисах: инъектируй провайдер опциональным параметром конструктора (прецеденты `WpfDispatcherService` с `Dispatcher?`, `WpfDialogHostService` с `Func<Window?>`), дефолт guarded через `Dispatcher.CheckAccess()`. Дефект пойман тестами в этом срезе.
