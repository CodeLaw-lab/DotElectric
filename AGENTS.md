...
| **Coverage** | **76.4% line-rate** | **90.04% line-rate (gate 80% достигнут)** |
| **CI coverage gate** | **75%** | **80% (факт 90.04%)** |
...
**Tests:** 2685 total (2684 passed, 1 pre-existing skip)
...
- **Tests:** 2685 total (2684 passed, 1 pre-existing skip)
- **Coverage:** 90.04% line-rate ✅
- **Build:** 0 errors, 0 warnings
- **CI/CD:** GitHub Actions — build + test + coverage-gate 80% + NuGet кэш
- **EditorViewModel:** ~792 строк (де-bloat R3.1: −410 строк, 25 forwarding-свойств удалено, 4 INPC-обработчика удалены; срез 1 кандидата 1: ещё 7 forwarding-свойств рамки выделения удалены)
- **Managers:** ZoomPan, Selection, Clipboard, ToolRegistry, Preview, InlineEdit, StatusBar, Grid, DirtyState
- **Tools:** ITool (без Name) + IEditorContext (29 членов, типизированные `PushTool`/`PopTool`/`ActivateTool`, без WPF ICommand; рамка выделения — методы `SetSelectionBox`/`ClearSelectionBox`; пан — жест роутера, не член интерфейса) + `ToolKind` + ResizeMath (чистые функции) + ShortcutRegistry
...
- **Тесты:** RenderRules 50 unit-тестов (точные значения, углы 45°/90°/135°/270° инвариантности slot-anchor, frozen-семантика, throw на неизвестный тип), +5 preview (точные anchor'ы, multiline поймал дефект), +8 печать (Y-позиция с высотой, многострочность, поворот, точные dash-значения), +1 конвертер.
