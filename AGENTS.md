...
| **Coverage** | **76.4% line-rate** | **89.89% line-rate (gate 80% достигнут)** |
| **CI coverage gate** | **75%** | **80% (факт 89.89%)** |
...
- **IEditorContext:** член `PanCanvas` удалён (**30 → 29 членов**) — единственный потребитель PanTool; метод на EditorViewModel остался публичным (роутер ходит через конкретный тип).
