...
| **Темы: структура + палитры (кандидат 8 обзора №5)** | **два словаря по 464 строки с ~96% совпадающих строк (структура стилей продублирована в обеих темах — класс дефекта «расхождение тем»); тема из Настроек применялась только при следующем запуске; мёртвые ресурсные ключи `MaterialFlat*`** | **один `ThemeStructure.xaml` (15 стилей, ни одного цвета) + палитры `DarkTheme.xaml`/`LightTheme.xaml` (по 22 цвета + 22 кисти; файлы различаются только значениями цветов); кисти в стилях — `DynamicResource` на палитру; механика смены темы и её тесты — нулевая правка; живая смена темы из Настроек (фикс скрытого дефекта); эффективная ресурсная поверхность запинена `ThemeResourceSurfaceTests` (STA); байт-в-байт кроме двух декларированных отклонений (живая смена из настроек; кисти в палитрах, а не в структуре — единственный контекст наследования незамороженного `Freezable`); тесты 2625 → 2632 (+7), покрытие 91.41%** |

**Build:** 0 errors, 0 warnings
**Tests:** 2632 total (2632 passed, 0 пропусков)
...
## Sprint — Кандидат 8 обзора №5: темы — одна структура стилей + две палитры (#172–#173, PR #175, 21.08.2026)

### Проблема
Кандидат 8 архитектурного обзора №5 (отчёт `architecture-review-20260820-1540.html`, раздел «Темы: два словаря, 96% одной строки»). Обе темы были словарями ресурсов по 464 строки, из которых ~96% строк совпадали: структура всех стилей продублирована в `DarkTheme.xaml` и `LightTheme.xaml`, различались только значения цветов — класс дефекта «структурная правка повторяется в двух местах, расхождение неизбежно». Тема из Настроек применялась только при следующем запуске (живая смена по F9 работала, из диалога настроек — нет). Словари несли мёртвые ключи `MaterialFlatButton`/`MaterialFlatToggleButton` (0 потребителей). Grilling 21.08.2026 (/grill-with-docs на develop после мержа кандидата 7, Q1–Q16) — все решения подтверждены по рекомендациям; спека #172 опубликована с верифицированным телом.

### Решение
- **`ThemeStructure.xaml`** (новый, `Resources/Styles`) — общая структура: все 15 стилей (4 именованных + типовой TabItem + 10 implicit), ни одного цвета; кисти в стилях берутся из палитры через `DynamicResource` (механизм живой подмены, прецедент фикса #123). Единственная структурная правка относительно исходных словарей: цветовой литерал в триггере шаблона TabItem (hover кнопки закрытия) заменён ссылкой `DynamicResource` на новую кисть `TabCloseButtonHoverBrush`.
- **`DarkTheme.xaml`/`LightTheme.xaml`** — палитры: по 22 цвета + 22 кисти (кисти ссылаются на цвета своей палитры); имена файлов сохранены — механика смены темы `IThemeService`/`ThemeDictionaryManager` и её тесты остались нулевой правкой. Файлы палитр различаются только значениями цветов.
- **App.xaml** — структура подключена перед палитрой; стартовая палитра — Dark (как прежде).
- Мёртвые ключи `MaterialFlatButton`/`MaterialFlatToggleButton` удалены (0 потребителей).
- **Живая смена темы из Настроек** (декларированное отклонение — фикс скрытого дефекта): после подтверждения диалога `MainViewModel.OpenSettings` применяет сохранённую тему через существующий `IThemeService.SetTheme` (+ свойство `Theme` главной модели); +2 теста на шве главной модели (подтверждение применяет, отмена не применяет).
- **Декларированное отклонение «кисти живут в палитрах, а не в структуре»** (решение 1 спеки предписывало кисти в структуре со ссылками на цвета через `DynamicResource`): разделяемый незамороженный `Freezable` имеет единственный контекст наследования WPF — `DynamicResource`-цвет обновляется только у одного владельца; смоук чисто-цветовой формы показал частичное перекрашивание (меню переключилось, тулбар и панели остались на старой теме). Форма «палитра = цвета + кисти, структура = стили с `DynamicResource` на кисти» перекрасила весь интерфейс живо в обе стороны. Наблюдаемый контракт сохранён: эффективный набор ключей и значения обеих тем те же, что до перестройки (пины).
- **Пины эффективной ресурсной поверхности** — новый `ThemeResourceSurfaceTests` (STA, словари главной сборки по pack-URI в потоке теста): точные значения всех цветов и кистей обеих палитр, точный состав палитр (22 Color + 22 кисти), структура — только стили (ни цветов, ни кистей), подмена палитры реальной механикой `FindThemeDictionary` (цвета следуют), композиция App.xaml (структура перед палитрой, палитра последняя).

### Тесты и верификация
- Тесты: 2625 → **2632** (2632 passed, 0 пропусков; распределение: приложение 2051 + Document.Tests 454 + Sheets.Tests 127) = +7 (5 пинов поверхности + 2 живой смены из настроек). Существующие тесты — без правок: ThemeServiceTests (26), ThemeDictionaryManagerTests (4 STA), пин #123 в MainWindowTests.
- Deletion-чеклист: в структуре 0 шестнадцатеричных цветовых литералов; `MaterialFlat*` — 0 вхождений в production/XAML; каждая из 22 кистей и каждый из 15 стилей определены ровно один раз; палитры не несут стилей.
- Сборка 0 ошибок / 0 предупреждений; слитый line-rate методом CI (ReportGenerator) **91.41%** (gate 80%).
- Смоук со снимками: старт в Light (сохранённая тема), F9 → полностью тёмный без смешанных зон, F9 → светлый; вкладки A0 + сетка в обеих темах (явный пользовательский цвет узлов #ffC0C0 сохранён при смене — явный цвет по Q8); разделители тулбара после F9 живые. Зона фикса #123 защищена STA-пином `NewSheetMenuContainerStyles_CarryThemeBrushes` (popup не захватывается фоновой автоматизацией).

### Не вошло / отложено (радар)
- Диалоги несут ту же `DynamicResource`-поверхность (STA-тесты SettingsView) — отдельной перестройки не требовалось.
- Документная библиотека и модели представления не менялись — срез чисто ресурсный.

**Common Mistakes (new):**
86. Кисти темы живут в палитре рядом со своими цветами, структура стилей ссылается на кисти через `DynamicResource`: разделяемый незамороженный `Freezable` (кисть) имеет единственный контекст наследования WPF — если вынести кисть с `DynamicResource`-ссылкой на цвет в общую структуру, при подмене палитры цвет обновится у одного владельца (частичное перекрашивание: поймано смоуком кандидата 8 обзора №5 при попытке формы «кисти в структуре»). Правильная форма: палитра = цвета + кисти этой палитры, структура = стили без цветов; перекраска живая по всему интерфейсу.

### Итоги
Поведение байт-в-байт, кроме двух декларированных отклонений (живая смена темы из Настроек; кисти в палитрах, а не в структуре). Класс дефекта «расхождение тем» устранён структурно: структура стилей существует в одном экземпляре. Слияние: PR #175 в develop (merge e34c3b4); спека #172 и тикет #173 закрылись автоматически.
